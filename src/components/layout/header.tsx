    ...
    <header className={cn("fixed top-0 z-50 w-full transition-all duration-300 h-14", headerDynamicClasses)}>
      <div className="container flex h-full items-center">
        ...
        <nav className="hidden md:flex flex-1 justify-center items-center gap-8">
            {navLinks.map((link) => (
                <Link key={link.label} href={link.href} className={cn("text-sm font-medium transition-colors whitespace-nowrap", linkDynamicClasses, pathname === link.href ? 'text-primary' : '')}>
                    {link.label}
                </Link>
            ))}
        </nav>
